...
public:
  ...
  [[nodiscard]] int mkdir(const char *relpath, mode_t mode = 0755) {
    char *path = LIBC_NAMESPACE::strdup(relpath);
    if (path == nullptr)
      return -1;

    if (!dirs_.push_back(path)) {
  ...
  [[nodiscard]] int touch(const char *relpath, mode_t mode = 0644) {
    ...
    if (!files_.push_back(path)) {
      tlog << "Not enough space in TestDir::files_\n";
      return -1;
    }
    int fd = LIBC_NAMESPACE::openat(fd_, relpath, O_RDONLY | O_CREAT, mode);
    if (fd < 0)
      return -1;
...
public:
  ...
  [[nodiscard]] bool create_test_dir(const char *name, TestDir &dst) {
    ...
    // Include the pid in case multiple builds of this test are running at once.
    test_dir_path += cpp::to_string(static_cast<int>(LIBC_NAMESPACE::getpid()));
